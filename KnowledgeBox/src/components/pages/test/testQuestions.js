...
const TestQuestionnaire = (props) => {
	...
	const { testId, title, user, stateId, catergoryId, testTitle, testTime, testInstructions } = props.route.params;
	...
	const onOptionPress = (index) => {
		const questions = JSON.parse(JSON.stringify(questionsList));
		if (questions[key].selectedAnswer.includes(index) && questions[key].isMultiple) {
			questions[key].selectedAnswer.splice(questions[key].selectedAnswer.indexOf(index), 1);
			questions[key].options[index - 1].isSelected = false;
		} else if (questions[key].selectedAnswer.includes(index) && !questions[key].isMultiple) {
	...
	const submitAnswers = () => {
		...
		for (let i = 0; i < questionsList.length; i++) {
			...
			answer.isCorrect = questionsList[i].selectedAnswer.toString() === questionsList[i].correctOption;
	...
	return (
		<SafeAreaView>
			{questionsList.length ? !introDone ? (
				<ContainerList
					title={title}
				...
				>
					...
					>
						<View flexDirection="row" justifyContent="space-between">
							<Text style={{ fontSize: 18 }}>Time Duration: {testTime} mins.</Text>
							<Text style={{ fontSize: 18 }}>
								Maximum Marks: {questionsList[questionsList.length - 1].maxMarks}
							</Text>
						</View>
						{testInstructions.split('\n').map((line) => {
							return (
								<Text key={line} style={{ color: 'grey', fontSize: 16 }}>
									* {line}
								</Text>
							);
						})}
				...
				<ScrollView style={{ marginBottom: 30 }}>
					...
					>
						...
						<View style={{ marginTop: 0 }}>
							<View style={{ alignItems: 'center' }}>
								{renderProgressBar(questionsList.length, questionsList[key].count)}
							</View>
							<View
								style={{
									position: 'absolute',
						...
						<View style={{ justifyContent: 'center', marginTop: 10 }}>
							<View>
								...
								>
									...
									<Text style={{ marginLeft: 10, fontSize: 25, color: 'white' }}> - </Text>
									<Text
										style={{
											marginLeft: 1,
											marginTop: 5,
							...
							>
								{languageFlag ? questionsList[key].questionLang : questionsList[key].question}
							</Text>
						</View>
					</LinearGradient>
					<View
					...
					>
						...
						>
							...
							>
								{!questionsList[key].isMultiple ? 'Single Choice' : 'Multiple Choice'}
							</Text>
						</View>
					</View>
					<View style={{ alignItems: 'center', margin: 10, marginTop: 15 }}>
						{questionsList[key][languageFlag ? 'optionsLang' : 'options'].map((option) => {
							return (
								<View
									key={option.id}
									style={{
										...styles.stayElevated,
										borderWidth: 3,
										borderColor: option.isSelected ? Constants.textColor1 : 'white'
									}}
								>
									<TouchableOpacity onPress={onOptionPress.bind(this, option.id)}>
					...
					<View flexDirection={'row'} style={{ marginHorizontal: 20, justifyContent: 'space-between' }}>
						...
						<View>
							<PButton
								title={questionsList[key].isMarked ? 'Unmark' : 'Mark for Review'}
