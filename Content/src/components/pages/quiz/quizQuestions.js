...
const QuizQuestionnaire = props => {
  const [questionsList, setQuestionsList] = useState([
    {
      id: 1,
      question:
        'This is the first question, are we going to complete it in next 6 days?',
      options: [
        {
          id: 1,
          value:
  ...
  const [chosen, setChosen] = useState([-1, -1, -1, -1]);
  const checkAnswer = (index, evt) => {
    var list = Array.from(chosen);
    list[key] = list[key] < 0 ? index : list[key];
    setChosen(list);
  };
        ...
        <View>
          <View style={{flex: 1, justifyContent: 'center'}}>
            ...
            <View style={{position: 'absolute', paddingLeft: 15}}>
              <TouchableOpacity
                onPress={() => {
                  Alert.alert('We have to go back.');
                }}>
              ...
              style={{
                ...
                borderColor:
                  chosen[key] != option.id
                    ...
                    : chosen[key] < 0
                    ? 'white'
                    : chosen[key] == questionsList[key].correctOption
                    ? '#3bb54a'
                    : '#e71918',
              }}>
              <TouchableOpacity onPress={checkAnswer.bind(this, option.id)}>
                <ElevatedView elevation={5} style={{borderRadius: 10}}>
                  <Text
                    style={{
                      ...styles.answerText,
                      color: chosen[key] < 0 ? Constants.textColor1 : '#858e71',
                    }}>
                    {option.value}
                  </Text>
                </ElevatedView>
